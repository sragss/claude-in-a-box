...
# Prepare startup command to change to the correct directory
STARTUP_CMD="cd '${STARTUP_DIRECTORY}' 2>/dev/null || cd /home/node; exec bash"

# Start Wetty with SSH connection and startup command
exec wetty \
  --host 0.0.0.0 \
  --port 3001 \
  --ssh-host "${SSH_HOST}" \
